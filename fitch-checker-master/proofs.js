...
var proofBeingExported = false;  // Copying this from above, not sure how to use it.


function processProofCheckResponse(text) { 
   // 'text' is result of 'echo json_encode($check_result);' from checkproof.php
...
function processProofExportResponse(text) { // What should we do with the output of the Export process?
   // 'text' is result of 'echo json_encode($export_result);' from exportproof.php
   if (!(proofBeingExported)) {
      return;
   }
	console.log("YY" + text);
   var res = JSON.parse(text);
   
   var restext = "Here's the exported proof in Fitch style:" + "<br /><br />" + res.fitch.join("<br />") + "<br /><br />" ;

   proofBeingExported.results.innerHTML = restext;
   proofBeingExported = false;
}
...
function makeProof(pardiv, pstart, conc) {
   ...
   p.exportButton = document.createElement("button");
   p.exportButton.type = "button";
   p.exportButton.innerHTML = "export proof";
   p.exportButton.myP = p;
   pardiv.appendChild(p.exportButton);
   p.exportButton.onclick = function() {
      this.myP.registerInput();
      this.myP.openline = 0;
      this.myP.jopen = false;
      this.myP.oInput = {};
      this.myP.displayMe();
      this.myP.startExportMe();
   }
   ...
   p.startCheckMe = function() {
      ...
      // fD.append("constructiveSettings", constructiveSettings.toString());  // HAVEN'T WORKED OUT HOW TO USE THIS YET
   ...
   p.startExportMe = function() {  // Exporting the proof to fitch.sty style for LaTeX processing
      proofBeingExported = this;
      this.results.innerHTML = '<img src="wait.gif" alt="[wait]" /> Exporting …';
      var fDe = new FormData();  // fDe = "form Data for export"
      fDe.append("predicateSettings", predicateSettings.toString());
      fDe.append("proofData", JSON.stringify(this.proofdata));
      fDe.append("wantedConc", this.wantedConc);
      fDe.append("numPrems", this.numPrems);
      AJAXPostRequest('exportproof.php', fDe, function(text) {
         processProofExportResponse(text);;
      });
   }
